...
export default function ProductCard({ id, image, title, department, price, discountPrice, colors = [], categoryId }) {
  const categories = useSelector((state) => state.product.categories)
  const category = categories.find((c) => c.id === categoryId)
  const gender = category?.gender === "k" ? "kadin" : "erkek"
  const categoryName = category?.title?.toLowerCase().replace(/\s+/g, "-") || "kategori"
  const slug = title?.toLowerCase().replace(/\s+/g, "-") || "product"
  const productLink = `/shop/${gender}/${categoryName}/${categoryId}/${slug}/${id}`
  ...
  return (
    <div className="w-full flex flex-col items-center">
      ...
      >
        {image ? (
          <img src={image} alt={title} className="w-full h-full object-contain object-center" loading="lazy" />
        ) : (
